...
## Override URLs if we are on local
## Point your local server's root to wiki's folder directly. FastCache won't 
## work if the URL is in the http://localhost/~username/wikifolder format.
...
$EnableUpload = 1;
# See 'Passwords and Permissions' section in this file for upload password.

SDV($UploadDir,'attach');
...
## Epubskin
$ActionSkin['epubview'] = 'epubview';

## Allow skins to be set with a GET ?skin= parameter, but limit what skins to 
## accept this way.

$allowed_skins = array('pmwiki', 'bootstrap-fluid', 'epubview');
...
if ($AuthId) {
    $EnableFastCache = false;
} else {
    $EnableFastCache = true;
}

## Keep this enabled, otherwise automatic content listings won't update after a
## page has been created or edited.
$FastCacheInvalidateAllOnUpdate = true;

$FastCacheDir = "$FarmD/pub/htmlcache";
...
include_once("$FarmD/cookbook/URLify.php");
